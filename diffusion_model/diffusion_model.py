...
plot_diffusion_steps = 300

batch_size = 64
image_size = 64

dataset_mean = torch.tensor([[[[0.4811]], [[0.4319]], [[0.3834]]]])
dataset_std = torch.tensor([[[[0.2591]], [[0.2506]], [[0.2526]]]])
...
def get_embeddings(t):
    frequencies = torch.exp(
        torch.linspace(
            math.log(embedding_min_freq),
            math.log(embedding_max_freq),
    ...
    print (t)
    result = torch.concat((torch.sin(angular_speeds * t), torch.cos(angular_speeds * t)), dim=-1)
    return result
...
def sinusoidal_embedding(x):
    ...
    ).to(x.device)  # Move frequencies to the same device as x

    angular_speeds = 2.0 * math.pi * frequencies
    embeddings = torch.cat(
        [torch.sin(angular_speeds * x), torch.cos(angular_speeds * x)], dim=-1
    )
    # Reshape the embedding to have spatial dimensions and match the batch size
    # The spatial dimensions should match the image size, and the channel dimension should be embedding_dimensions
    return embeddings.view(-1, embedding_dimensions, 1, 1).repeat(1, 1, 64, 64) # Assuming image_size is 64
...
class DiffusionModel(nn.Module):
    ...
    def plot_images(self, epoch=None, logs=None, num_rows=3, num_cols=6, power=1.0, diffusion_steps=100):
        ...
        for row in range(num_rows):
            for col in range(num_cols):
                ...
                plt.axis("off")
        plt.tight_layout()
        plt.savefig("generated.png", bbox_inches="tight", dpi=100)
        plt.close()
